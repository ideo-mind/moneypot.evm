...
// Blockscout error parsing utilities
const parseBlockchainError = (
  error: any
): ErrorReport["blockscoutMetadata"] => {
  const currentChain = getCurrentChainConfig()

  const metadata: ErrorReport["blockscoutMetadata"] = {
    chainId: currentChain.id,
    explorerUrl: currentChain.explorerUrl,
    apiUrl: currentChain.apiUrl,
  }
  ...
  if (error?.message) {
    ...
    if (message.includes("gas") || message.includes("out of gas")) {
      metadata.errorType = "gas_error"
      metadata.errorCode = "GAS_ERROR"
    }
    ...
    if (message.includes("user rejected") || message.includes("user denied")) {
      metadata.errorType = "user_rejection"
      metadata.errorCode = "USER_REJECTED"
    }

    // Contract revert
    if (message.includes("revert") || message.includes("execution reverted")) {
      metadata.errorType = "contract_revert"
      metadata.errorCode = "CONTRACT_REVERT"
    }
  ...
  if (error?.transactionHash) {
    metadata.transactionHash = error.transactionHash
  }

  if (error?.hash) {
    metadata.transactionHash = error.hash
  }

  // Extract block number if present
  if (error?.blockNumber) {
    metadata.blockNumber = error.blockNumber
  }

  // Extract gas used if present
  if (error?.gasUsed) {
    metadata.gasUsed = error.gasUsed.toString()
  }
...
class ErrorReporter {
  ...
  private createErrorPayload(
    ...
    const category = categorize(data.message)

    // Parse Blockscout metadata for blockchain errors
    let blockscoutMetadata: ErrorReport["blockscoutMetadata"] | undefined
    if (category === "blockchain" && data.error) {
      blockscoutMetadata = parseBlockchainError(data.error)
    }
